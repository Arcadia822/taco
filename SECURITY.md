...
- Markdown, stored block HTML, Mermaid output, collaboration payloads, and unknown text are treated as untrusted content. Supported editor content is rebuilt through an allowlist before it enters live DOM, passive remote images are not loaded, and malformed collaboration units are rejected before store mutation.
- Opening an HTML prototype intentionally executes that prototype only after an explicit click in a separate opaque-origin `data:` page with no opener. The prototype may use its own network access, but it does not inherit Taco-origin storage, channels, permissions, DOM, or the `window.taco` API.
- A Taco with `collab.key`, `collab.ownerPriv`, or delegated invitation private material is a bearer credential. Run `taco validate <file.taco.html> --json` locally before Agent inspection or external transmission. Removing credential fields is not revocation; use Reset Access after suspected exposure.
- The machine-readable `taco-security-version` belongs to each self-contained artifact. Updating the repository or extension does not patch Taco files already copied elsewhere; repack them from canonical files to replace the runtime.
