...
- Every HTML/HTM file must have `files[].sourceUrl`. CLI-created Taco files use an absolute `file:` URL with no authority, credentials, query, or fragment whose decoded pathname ends in the validated `files[].path`. The committed showcase shell alone uses the exact `../<files[].path>` reference so its generated bytes do not depend on the checkout directory; the runtime resolves that reference only when the Taco itself has a local `file:` URL. Non-HTML files must not carry this field. Invalid bundles enter Recovery mode instead of receiving a generated `data:` or Blob fallback.
