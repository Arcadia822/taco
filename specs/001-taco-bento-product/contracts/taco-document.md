...
Stage navigation adds no bundle fields. Core files and Speckit convention files are identified by relative path; other Markdown declares its membership from a ``**Taco scope**: spec|plan|tasks`` enum in its own content. That internal property is preserved in the canonical Markdown but not shown in the rendered document. The directory structure continues to be derived from `path`.
...
- `pack <feature-dir>`: read UTF-8 text files, record a `sourceHash`, and create a Taco.
