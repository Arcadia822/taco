...
- **FR-003**: The layout must first split the left file sidebar from the workspace; below the workspace header, the central document area and the in-document outline/comments local aux area follow. The left side organizes files into structured groups and preserves the physical directory hierarchy. When a top-level `navigation` manifest is present, custom groups, ordering, and entry declarations are derived directly from it with undeclared files retained in Unassigned; when absent, files default to the three standard stages (Specify, Plan, Tasks) according to core and convention rules.
- **FR-004**: The default file must prefer `navigation.entry` when defined and valid. When unspecified or invalid, it must prefer `README.md` at the feature root, then fall back to `spec.md` when no root README exists, followed by the first Markdown file or first bundled file.
