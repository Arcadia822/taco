...
- `npm run check`: 39 Vitest files / 385 tests passed; TypeScript, format check, dual builds, and shell gates passed. After the Lite startup fix, the empty Lite shell is **201,384 bytes** (<230,400); the built Lite shell with its sample document is 306,074 bytes; the built Complete shell is 2,836,314 bytes.
...
- Lite startup regression: the async rich-editor promotion previously appended the rendered Markdown below its temporary source editor, while the selected root document remained hidden in a closed Unassigned group. The rich editor now replaces the temporary source surface and opens the selected group without discarding a user's subsequent collapse choice. A `file:` browser check found one visible rich editor, zero source editors, and the selected `spec.md` row in an open group for both Lite and Complete.
