...
The rightmost header button uses an open/close sidebar icon to toggle the outline/comment panel without changing its active tab or discarding an unsubmitted draft. On desktop, closing the panel releases its reading-space width and remembers the choice per document for the current browser session. Narrow screens start with a closed drawer and do not overwrite the desktop preference. Use the panel's close button or Escape to close it; active dialogs, menus, and editor key handlers take precedence. Starting a comment opens its composer, and clicking an existing inline comment highlight opens and focuses the matching thread. These panel preferences are local UI state, not saved document edits.
