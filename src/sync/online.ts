...
// Taco online transport — E2EE WebSocket to the blind relay
...
import type { TacoSyncDoc } from '../store.ts'
import type { Op, SyncStateJSON } from './crdt.ts'
import type { Frame, RefusalCode, TacoSyncSession, Transport } from './session.ts'
import { storageGet, storageSet } from '../kernel/storage.ts'

/** Relay selection is a build/deployment concern, not a Share-menu control. */
export const DEFAULT_SYNC_HOST = (import.meta.env.VITE_TACO_SYNC_URL || '').trim().replace(/\/$/, '')
...
export async function deviceIdentity(docId: string): Promise<{ pub: string; priv: string }> {
  const k = `taco-member-${docId}`
  try {
    const saved = storageGet(k)
    if (saved) return JSON.parse(saved)
  } catch { /* storage unavailable → ephemeral identity */ }
  const id = await mintKeypair()
  storageSet(k, JSON.stringify(id))
  return id
}
 ...
 */
export async function mintCollab(host = syncHost()): Promise<CollabCreds> {
  ...
  if (!normalizedHost) throw new Error('Taco relay is not configured')
  return {
    room: `${normalizedHost}/d/w${b64u.enc(commit)}`,
    key: mintRoomKey(),
    on: true,
    v: 2,
    owner: kp.pub,
...
export function syncHost(): string {
  return (storageGet('taco-sync-url') || DEFAULT_SYNC_HOST).replace(/\/$/, '')
}

export function setSyncHost(value: string): string {
  const normalized = value.trim().replace(/\/$/, '')
  if (normalized && !/^wss?:\/\//.test(normalized)) throw new Error('Relay URL must start with ws:// or wss://')
  storageSet('taco-sync-url', normalized)
  return normalized
}

export const offlineEnabled = (): boolean => storageGet('taco-offline') === '1'
...
export class OnlineTransport implements Transport {
  ...
  constructor(
    ...
    private hooks: {
      onSnap: (doc: TacoSyncDoc, state: SyncStateJSON) => void
      getSnapshot: () => { doc: TacoSyncDoc; state: SyncStateJSON }
      onOpen: () => void
  ...
  private handleRefusal(env: RefusedEnv) {
    ...
    if (env.code !== 'too-large' && env.code !== 'storage-failed' && env.code !== 'room-full') {
      ...
      console.warn('[taco-sync] relay refused a frame:', env.code)
      return
    }
    const culprit = this.matchRefused(env)
    if (culprit) this.awaitingAck = this.awaitingAck.filter((o) => o !== culprit)
    console.warn(`[taco-sync] relay refused a frame (${env.code})`, env)
  ...
  private async onEnvelope(text: string) {
    ...
    if (env.ctl === 'revoked') {
      if (env.p && env.p === this.myPub) {
        console.info('[taco-sync] this copy’s access was revoked by the owner')
        this.close()
      }
      return
    }
    ...
    if (env.snap === 1) {
      const s = payload as { doc: TacoSyncDoc; state: SyncStateJSON }
  ...
  close() {
    ...
    this.ws?.close()
    this.ws = null
    const session = sessionByTransport.get(this)
    if (session) activeBySession.delete(session)
    sessionByTransport.delete(this)
    this.setStatus('closed')
  }
...
const activeBySession = new WeakMap<TacoSyncSession, OnlineTransport>()
const sessionByTransport = new WeakMap<OnlineTransport, TacoSyncSession>()

export function onlineTransport(session: TacoSyncSession): OnlineTransport | null {
  return activeBySession.get(session) ?? null
}
...
export function sharingOn(store: TacoStore): boolean {
...
export function joinFromDoc(session: TacoSyncSession, store: TacoStore): OnlineTransport | null {
  if (offlineEnabled()) return null // the hard no-network switch wins over everything
  const current = activeBySession.get(session)
  if (current) return current
  if (!sharingOn(store)) return null
  const transport = session.addTransport((docId, onFrame) => {
    ...
    if (!collab?.room || !collab.key || collab.on === false || store.doc.docId !== docId) {
      activeBySession.delete(session)
      return new NullTransport()
    }
    activeBySession.get(session)?.close()
    ...
    const online = new OnlineTransport(collab.room, collab.key, docId, onFrame, {
    ...
    activeBySession.set(session, online)
    sessionByTransport.set(online, session)
    return online
  })
  return transport instanceof OnlineTransport ? transport : null
}
...
export async function startSharing(session: TacoSyncSession, store: TacoStore): Promise<OnlineTransport | null> {
 ...
 */
export function disconnectOnline(session: TacoSyncSession) {
  const current = activeBySession.get(session)
  if (current) {
    session.removeTransport(current)
    activeBySession.delete(session)
    sessionByTransport.delete(current)
  }
}

/** flip sharing off and disconnect. Credentials stay — copies saved during
 * the session can rejoin if sharing is turned back on. */
export function stopSharing(session: TacoSyncSession, store: TacoStore) {
  const current = activeBySession.get(session)
  if (current) {
    session.removeTransport(current)
    activeBySession.delete(session)
...
export async function rotateKeys(session: TacoSyncSession, store: TacoStore) {
  let previousHost = syncHost()
  if (!previousHost && store.doc.collab?.room) {
    try { previousHost = new URL(store.doc.collab.room).origin } catch { /* invalid rooms fail in mintCollab */ }
  }
  stopSharing(session, store)
  const fresh = await mintCollab(previousHost)
  store.commit(() => {
